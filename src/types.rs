...
impl Commands {
    pub fn build_yaml(&self, cur_date_time: &str) -> (String, String) {
        match self {
            Commands::SQS(sqs_command) => {
                match sqs_command.command.as_ref().unwrap() {
                    SQSCommand::Download { queue, channel } => ("download_sqs".to_string(), build_download_yaml(cur_date_time, channel, &queue)),
                    SQSCommand::Reprocess { src_queue, dst_queue, channel } => ("reprocess_sqs".to_string(), build_reprocess_yaml(cur_date_time, channel, &src_queue, &dst_queue)),
                    SQSCommand::Purge { queue } => ("purge_sqs".to_string(), build_purge_yaml(cur_date_time, &queue)),
                }
            }
        }
    }
...
spec:
  ...
  templates:
    ...
    "#)));
        ...
        let reprocess_command = Some(SQSCommand::Download { queue, channel });
        let sqs_command = SQS { command: reprocess_command };

        let cli = Cli {
            command: Commands::SQS(sqs_command),
            squad: String::from(""),
            root_directory: String::from(""),
            envs: Vec::new(),
        };
        let (act_relative_name, act_content) = cli.command.build_yaml(&cur_date_time);
    ...
    #[test]
    fn build_yaml_for_purge() {
        let exp = (String::from("purge_sqs"),
                   String::from(format!(
                       r#"apiVersion: argoproj.io/v1alpha1
kind: Workflow
metadata:
...
spec:
  ...
  templates:
    ...
    "#)));
        ...
        let reprocess_command = Some(SQSCommand::Reprocess { channel, src_queue, dst_queue });
        let sqs_command = SQS { command: reprocess_command };
